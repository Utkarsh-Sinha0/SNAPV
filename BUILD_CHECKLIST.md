# BUILD_CHECKLIST.md
# SnapVault — Skills Inventory + Release Ledger
...
# RELEASE PASS — 2026-03-17
# Verdict: release-ready for Chrome, Edge, and Firefox package submission.
# Verified ship gates:
# - `npm run typecheck`
# - `npm run test:run`
# - `npm run test:audit:pixel-payload`
# - `npm run test:e2e:extension`
# - `npm run zip:all`
# - `npm run validate:artifacts`
# Firefox note:
# - `npm run test:firefox:package` enforces the approved 3-warning Mozilla lint baseline.
# - Those warnings come from bundled Preact / Transformers / ONNX Runtime output, not
#   first-party SnapVault source.
...
## Core (must know well)
- [x] **TypeScript strict mode** — `noImplicitAny`, `strictNullChecks`, `exactOptionalPropertyTypes`.
      If you don't understand why `string | undefined` ≠ `string`, learn this first.
- [x] **Chrome Extension MV3 mental model** — service worker lifecycle, message passing,
      `chrome.tabs`, `chrome.storage`, `chrome.scripting`. Read the MV3 migration guide.
- [x] **WXT 0.20.19** — file-based entrypoints, `wxt.config.ts`, cross-browser build flags,
      `browser.*` abstraction layer. Read the WXT docs intro + cross-browser guide.
- [x] **Canvas API in an offscreen context** — `OffscreenCanvas`, `createImageBitmap`,
      `canvas.convertToBlob`, `ctx.drawImage`. Know that these work in offscreen docs
      but NOT in service workers.
- [x] **`chrome.offscreen` API** — `createDocument`, `hasDocument`, `closeDocument`, reason
      flags (`DOM_PARSING`, `BLOBS`). Read the Chrome offscreen API reference page.
- [x] **Vitest** — `describe`, `it`, `expect`, `vi.mock`, `beforeEach`. Know how to run a
      single test file and read coverage output.
- [x] **Playwright extension testing** — loading an unpacked extension in Playwright,
...
- [x] **Transformers.js 3.x** — `pipeline()`, `env.localModelPath`, `env.allowRemoteModels`,
      WASM/ONNX backend setup. Read the Transformers.js "use in browser extension" guide.
- [x] **pdf-lib** — `PDFDocument.create()`, `addPage()`, `embedPng()/embedJpg()`,
      `save()`. Read the pdf-lib README examples.
- [x] **webextension-polyfill** — why you need it for Firefox MV2 compat, how WXT wraps it.
- [x] **WebGPU basics** — `navigator.gpu.requestAdapter()`, null-check fallback pattern.
      You only need to know how to detect availability and fall back gracefully.
- [x] **Stripe Checkout (server-side only)** — `stripe.checkout.sessions.create()`,
      webhook verification with `stripe.webhooks.constructEvent()`. No SDK goes in extension.
- [x] **JPEG iterative quality search** — binary search or step-down loop on
...
- [x] **1.01** `npm create wxt@latest` — initialise project with TypeScript template.
      TEST: `npm run typecheck` exits 0 with zero errors.

- [x] **1.02** Pin WXT to `0.20.19` in `package.json`. Add `engines: { node: ">=20" }`.
      TEST: `node -e "require('./package.json').devDependencies.wxt"` prints `0.20.19`.

- [x] **1.03** Create `wxt.config.ts` with `browser` and `manifestVersion` cross-browser
...
- [x] **1.04** Add all required permissions to `wxt.config.ts`:
...
- [x] **1.07** Add `ads_sandbox.html` to `manifest.sandbox.pages` in `wxt.config.ts`.
      TEST: `dist/chrome/manifest.json` has `"sandbox": { "pages": ["ads_sandbox.html"] }`.

- [x] **1.08** Create `src/content/index.ts` as a WXT content script entrypoint with
...
- [x] **1.09** Scaffold all `src/shared/` module files as empty exports:
...
- [x] **1.10** Create `src/assets/sponsor.json` with placeholder sponsor data.
...
- [x] **2.01** Define `ExportSpec` interface with all fields from API_SPECIFICATIONS.md §1.
      TEST: TypeScript compiles. A valid `ExportSpec` object satisfies the type.
            An object missing `format` causes a TS error.

- [x] **2.02** Define `CaptureMetadata` interface (cssWidth, cssHeight, devicePixelRatio,
      screenLeft, screenTop, lightMode, capturedAt).
      TEST: TypeScript compiles. An object with wrong field type causes a TS error.

- [x] **2.03** Define `FeasibilityResult` interface.
      TEST: TypeScript compiles.

- [x] **2.04** Define `RedactAnnotation` interface and `RedactAnnotationType` union.
      TEST: TypeScript compiles. `'face' | 'email' | ...` covers all PRD-listed types.

- [x] **2.05** Define `ExportSpecPreset` interface with `snapvault_preset: '1.0'` sentinel.
      TEST: TypeScript compiles. An object with `snapvault_preset: '2.0'` causes a TS error.

- [x] **2.06** Define `LicenseState` interface
...
- [x] **2.07** `assertNoPixelPayload(payload: unknown): void` — throws `Error` if payload
      contains a `dataURL` field (string starting with `data:`).
      TEST: `assertNoPixelPayload({ dataUrl: 'data:image/png;base64,...' })` throws.

- [x] **2.08** `assertNoPixelPayload` throws if payload contains an `ImageData` instance.
      TEST: `assertNoPixelPayload({ img: new ImageData(1,1) })` throws.

- [x] **2.09** `assertNoPixelPayload` throws if payload is an `ArrayBuffer`.
      TEST: `assertNoPixelPayload(new ArrayBuffer(8))` throws.

- [x] **2.10** `assertNoPixelPayload` does NOT throw for licensing metadata payloads.
      TEST: `assertNoPixelPayload({ installationId: 'abc', plan: 'pro' })` does not throw.

- [x] **2.11** `assertNoPixelPayload` does NOT throw for empty objects or primitives.
      TEST: `assertNoPixelPayload({})` and `assertNoPixelPayload('hello')` do not throw.

- [x] **2.12** `assertNoPixelPayload` throws if payload contains a nested `dataUrl`.
      TEST: `assertNoPixelPayload({ meta: { dataUrl: 'data:...' } })` throws.

## 2C — DPI utilities (`src/shared/dpi.ts`)

- [x] **2.13** `toDevicePixels(cssValue: number, dpr: number): number` — returns
      `cssValue * dpr`.
      TEST: `toDevicePixels(100, 2)` === 200. `toDevicePixels(100, 1)` === 100.

- [x] **2.14** `toCssPixels(physicalValue: number, dpr: number): number` — returns
      `physicalValue / dpr`.
      TEST: `toCssPixels(200, 2)` === 100. `toCssPixels(150, 1.5)` === 100.

- [x] **2.15** `applyDpiPolicy(width: number, height: number, dpr: number,
...
- [x] **2.16** `isHiDpi(dpr: number): boolean` — returns true when `dpr > 1`.
      TEST: `isHiDpi(2)` true. `isHiDpi(1)` false. `isHiDpi(1.5)` true.

## 2D — Export Spec validation (`src/shared/export-spec.ts`)

- [x] **2.17** `validateExportSpec(spec: unknown): ExportSpec` — returns a typed
...
- [x] **2.18** `validateExportSpecPreset(raw: unknown): ExportSpecPreset` — validates
...
- [x] **2.19** `getDefaultExportSpec(): ExportSpec` — returns a hardcoded safe default
      (PNG, no resize, css1x, simple filename template).
      TEST: Return value passes `validateExportSpec`.

- [x] **2.20** `DEFAULT_PRESETS: ExportSpecPreset[]` — array of built-in presets
...
- [x] **2.21** `checkFeasibility(spec: ExportSpec, metadata: CaptureMetadata):
...
- [x] **2.22** Upscale ratio check — blocking. If requested output width > source
...
- [x] **2.23** Estimated bytes check — blocking. Estimate PNG bytes as
...
- [x] **2.24** PDF page crop risk warning. If format is `pdf` and requested height
      > A4 equivalent at the given DPI, add to `warnings`.
      TEST: format=pdf, height=20000 at 96dpi → warning present.

- [x] **2.25** CPU-seconds estimate — warning. Estimate as
...
- [x] **2.26** HiDPI info flag (Tier 1 only). If `metadata.devicePixelRatio > 1` AND
...
- [x] **2.27** Light mode already active note. If `metadata.lightMode === true`, suppress
      the CPU-seconds warning (user already accepted the trade-off).
      TEST: Same 8000×6000 input with `lightMode: true` → no CPU warning.

- [x] **2.28** `estimatedBytesRange` is always populated as `[min, max]` regardless of
...
- [x] **2.29** `stitchSegments(segments: ImageBitmap[], stepPx: number,
...
- [x] **2.30** Two-segment stitch without overlap correction (lightMode=true).
...
- [x] **2.31** Two-segment stitch WITH overlap correction (lightMode=false).
...
- [x] **2.32** N-segment stitch — applies stitch pairwise accumulating into one canvas.
      TEST: 5 segments of 100×200, stepPx=180, lightMode=true →
            output height = 200 + (4 × 180) = 920px.

- [x] **2.33** `computeRowHash(canvas: OffscreenCanvas, y: number): number` —
...
- [x] **2.34** `encodePng(canvas: OffscreenCanvas): Promise<Blob>` — wraps
      `canvas.convertToBlob({ type: 'image/png' })`.
      TEST (mock): `convertToBlob` called with correct type. Returns a Blob.

- [x] **2.35** `encodeJpegAtQuality(canvas: OffscreenCanvas, quality: number):
      Promise<Blob>` — wraps `canvas.convertToBlob({ type: 'image/jpeg', quality })`.
      TEST (mock): quality=0.8 → correct options passed.

- [x] **2.36** `encodeJpegTargetSize(canvas: OffscreenCanvas, targetBytes: number,
...
- [x] **2.37** `encodeJpegTargetSize` returns the smallest result achieved even if
...
- [x] **2.38** `buildPdf(pages: Blob[], spec: ExportSpec): Promise<Uint8Array>` —
...
- [x] **2.39** Multi-page PDF — each blob becomes its own page at A4 or spec dimensions.
      TEST: 3 blobs → PDF has 3 pages.

## 2I — Redact apply (`src/shared/redact.ts`)

- [x] **2.40** `applyRedactAnnotations(canvas: OffscreenCanvas,
...
- [x] **2.41** Annotations where `userReviewed: false` are NOT applied.
...
- [x] **2.42** `detectEmail(text: string): boolean` — regex match.
      TEST: `'user@example.com'` → true. `'notanemail'` → false. `'a@b'` → false.

- [x] **2.43** `detectPhone(text: string): boolean` — E.164 + common national formats.
      TEST: `'+1-800-555-0100'` → true. `'hello'` → false.

- [x] **2.44** `detectCreditCard(text: string): boolean` — regex + Luhn algorithm.
      TEST: `'4532015112830366'` → true (valid Luhn). `'1234567890123456'` → false (invalid Luhn).

- [x] **2.45** `luhnCheck(digits: string): boolean` — standalone Luhn implementation.
      TEST: Known valid cards → true. Tampered digits → false.

- [x] **2.46** `detectApiKey(text: string): boolean` — heuristic: length ≥ 20,
      alphanumeric + `-_`, Shannon entropy ≥ 3.5 bits/char.
      TEST: A real-looking API key string → true. A normal English word → false.

- [x] **2.47** `detectSsn(text: string): boolean` — regex `\d{3}-\d{2}-\d{4}`.
      TEST: `'123-45-6789'` → true. `'123-456-789'` → false.

- [x] **2.48** `scanTextNode(text: string): RedactAnnotationType[]` — runs all detectors,
...
- [x] **3.01** `ensureOffscreenDocument(): Promise<void>` in
...
- [x] **3.02** `closeOffscreenDocument(): Promise<void>` — calls
      `chrome.offscreen.closeDocument()`. Catches and ignores "no document" errors.
      TEST (mock): `closeDocument` called exactly once.

- [x] **3.03** Idle timer: after each offscreen operation, a 30s timeout schedules
...
- [x] **3.04** `nukeOffscreenMemory(): Promise<void>` — cancels idle timer, calls
      `closeOffscreenDocument` immediately.
      TEST (mock + fake timer): pending timer is cancelled; close called immediately.

- [x] **3.05** `offscreen-adapter.ts` — `sendToHeavyWorker<T>(msg): Promise<T>`:
...
- [x] **3.06** Chromium offscreen runtime listener (`src/offscreen/runtime.chromium.ts`)
      — receives `OFFSCREEN_CLEAR_MEMORY`, clears any held references, and routes heavy
      work into the shared core in `src/shared/heavy-worker-service.ts`.
      TEST (jsdom): Send `OFFSCREEN_CLEAR_MEMORY` → listener responds `{ ok: true }`.

- [x] **3.07** Correlation IDs — every offscreen request includes a unique `id` (UUID).
...
- [x] **4.01** `generateInstallationId(): string` — generates a random UUID v4, stores
...
- [x] **4.02** `CAPTURE_VISIBLE` handler in SW — calls `chrome.tabs.captureVisibleTab`,
...
- [x] **4.03** `STORE_CAPTURE_DATA_URL` handler — stores `{ dataUrl, metadata }` in
...
- [x] **4.04** `GET_CAPTURE_DATA_URL` handler — returns from in-memory Map first,
      falls back to `chrome.storage.local`.
      TEST (mock): In-memory hit → storage NOT queried.
                   Cache miss → storage queried.

- [x] **4.05** `DELETE_CAPTURE` handler — removes from both in-memory Map and
      `chrome.storage.local`.
      TEST (mock): Both removal paths called.

- [x] **4.06** `PURGE_EXPIRED_CAPTURES` handler — lists all `capture:*` keys in
...
- [x] **4.07** SW calls `PURGE_EXPIRED_CAPTURES` once at startup.
      TEST (mock): handler invoked within 1 tick of SW `install` event.

- [x] **4.08** `CAPTURE_REGION` handler — when called without a rect, starts the
      interactive content-script region overlay. When the content script returns a rect,
      captures the visible tab and crops it through the heavy-worker encode path.
      TEST (mock): overlay-start message sent first; crop path forwards exact coordinates.

- [x] **4.09** `CAPTURE_FULLPAGE` handler — orchestrates scroll steps:
...
- [x] **4.10** `CAPTURE_FULLPAGE` sends `lightMode` from current settings in
      `CaptureMetadata`.
      TEST (mock, lightMode=true): metadata.lightMode === true in OFFSCREEN_STITCH payload.

- [x] **4.11** `CAPTURE_SCROLL_CONTAINER` handler — receives `selector` string,
...
- [x] **4.12** `NUKE_ALL_CAPTURES` handler — calls `nukeOffscreenMemory()`, then
...
- [x] **5.01** `APPLY_EXPORT_SPEC` handler — retrieves dataURL from cache, sends
...
- [x] **5.02** `OFFSCREEN_ENCODE` handler in the shared heavy-worker core — applies
      DPI policy via `applyDpiPolicy`, resizes OffscreenCanvas if needed, routes to
      `encodePng` / `encodeJpegAtQuality` / `encodeJpegTargetSize` based on spec,
      and returns a data URL artifact to Chromium offscreen or the Firefox background page.
...
- [x] **5.03** `OFFSCREEN_ENCODE` — DPI normalization gated. If `dpiPolicy='css1x'`
...
- [x] **5.04** `OFFSCREEN_BUILD_PDF` handler — calls `buildPdf(pages, spec)`, returns
      result as base64 or Blob.
      TEST: 2-page input → PDF Uint8Array with 2 pages (mock pdf-lib).

- [x] **5.05** `EXPORT_DOWNLOAD` handler — receives encoded Blob/dataURL from
...
- [x] **5.06** Filename template resolution — `{date}` → `YYYY-MM-DD`,
...
- [x] **5.07** `EXPORT_CLIPBOARD` handler — encodes to PNG Blob, writes to clipboard
...
- [x] **5.08** `CHECK_FEASIBILITY` handler — calls `checkFeasibility(spec, metadata)`,
...
- [x] **6.01** `<CaptureButtons>` component — renders three buttons:
...
- [x] **6.02** `<ExportSpecPicker>` component — renders format selector (PNG/JPEG/PDF),
...
- [x] **6.03** `<FeasibilityBanner>` component — takes `FeasibilityResult` prop.
...
- [x] **6.04** `<HiDpiBanner>` sub-component — renders non-blocking info strip with
      upgrade CTA when `hiDpiWarning` is true.
      TEST: Renders with correct text. Does NOT render when `hiDpiWarning: false`.

- [x] **6.05** Popup root — on mount, sends `CHECK_FEASIBILITY` with stored spec +
...
- [x] **6.06** `<ActionBar>` component — shown after a capture is available.
...
- [x] **6.07** `<ExportReceipt>` component — shown briefly after successful download.
...
- [x] **11.01** `generateInstallationId` (already done in 4.01) — confirm it's used as
      the ID in all licensing calls.
      TEST: `START_LICENSE_CHECKOUT` payload contains the stored `installationId`.

- [x] **11.02** `START_LICENSE_CHECKOUT` handler — sends
...
- [x] **11.03** `SYNC_LICENSE` handler — sends `POST /v1/licensing/sync`,
      stores returned `LicenseState` in `chrome.storage.local`.
      TEST (mock fetch): response stored; `licenseStatus` updated.

- [x] **11.04** `GET_LICENSE_STATE` handler — returns current `LicenseState` from
      storage without a network call.
      TEST: Returns stored state immediately.

- [x] **11.05** License sync on popup open — popup sends `SYNC_LICENSE` once per day
...
- [x] **11.06** Licensing backend `POST /v1/licensing/checkout` — creates Stripe
...
- [x] **11.07** Licensing backend webhook handler — verifies Stripe signature,
...
- [x] **11.08** `ads_sandbox.html` renders `sponsor.json` card when no Carbon script
...
- [x] **12.01** `src/shared/browser.ts` — exports `isFirefox(): boolean` based on
      `navigator.userAgent` or `typeof browser !== 'undefined'`.
      TEST: in Chrome user-agent context → false. Firefox UA string → true.

- [x] **12.02** Firefox background page (`src/background/background-page.ts`) —
...
- [x] **12.03** `offscreen-adapter` Firefox path — `sendToHeavyWorker` routes to
...
- [x] **12.04** `npm run build:firefox` — zero TypeScript errors; manifest has
      `manifest_version: 2`; no `offscreen` permission; package validation passes with the
      approved Firefox lint baseline.
      TEST: build exits 0; `dist/firefox/manifest.json` checked programmatically.

- [x] **12.05** Firefox package/runtime contract — build, artifact validation, and
      `web-ext lint` pass for the MV2 package while Vitest covers the background-page path.
      TEST: `npm run test:firefox:package` exits 0.

---

# PART 13 — SECURITY AUDIT + PERFORMANCE

- [x] **13.01** Static pixel payload audit — script that greps for `fetch(`, `XHR`,
...
- [x] **13.02** Network intercept test — Playwright test that intercepts all network
...
- [x] **13.03** ML network intercept test — Playwright test during `RUN_ML_REDACTION`.
      Assert zero network requests.
      TEST: No fetch to any URL during ML inference.

- [x] **13.04** `env.allowRemoteModels === false` assertion — Vitest test that imports
      `ml-redaction.ts` and checks the env value after import.
      TEST: Value is `false` after module load.

- [x] **13.05** Offscreen lifecycle integration — Playwright extension test:
...
- [x] **13.06** "Nuke everything" integration — Playwright:
...
- [x] **13.09** Idle heap probe decision — do NOT gate release on
      `performance.memory.usedJSHeapSize`, because extension contexts do not expose stable,
      portable heap metrics across Chromium, Edge, and Firefox package validation.
      TEST: startup budgets, capture budgets, offscreen lifecycle, and artifact validation
            remain the enforced release perf gates.

- [x] **13.10** Ads isolation verification — Playwright: inspect Options page DOM.
...
- [x] **14.01** Full Tier 1 flow — Playwright e2e:
...
- [x] **14.02** Full Tier 2 flow — Playwright e2e (Pro license mocked):
...
- [x] **14.03** Preset share flow — Playwright e2e:
...
- [x] **14.04** HiDPI info banner flow — Playwright e2e:
...
- [x] **14.05** Multi-capture board gate — Playwright e2e (free tier):
      Attempt to open board. Assert error state shown. Assert no captures processed.
      TEST: Error message contains "Pro" or "upgrade".

- [x] **14.06** `npm run zip:all` + `npm run validate:artifacts` produce valid Chrome,
      Edge, and Firefox release archives with no extraneous files (no source maps in
      production, no `.ts` files, no `node_modules`).
