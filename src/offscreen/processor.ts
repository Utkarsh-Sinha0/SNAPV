import { applyDpiPolicy } from '../shared/dpi';
import { encodeJpegAtQuality, encodeJpegTargetSize, encodePng } from '../shared/encode';
import { buildPdf } from '../shared/pdf';
import { stitchSegments } from '../shared/stitch';
import { resolveExportDimensions } from '../shared/export-spec';
import { runMlRedaction } from './ml-redaction';
import type {
  CaptureMetadata,
  ExportArtifact,
  ExportSpec,
...
async function encodeCanvasForSpec(
  ...
  const pdfBytes = await buildPdf([pngBlob], spec);
  return {
...
async function exportDataUrl(
  ...
  return encodeCanvasForSpec(canvas, spec);
}

async function buildPdfArtifact(pages: string[], spec: ExportSpec): Promise<ExportArtifact> {
  const blobs = await Promise.all(pages.map((page) => dataUrlToBlob(page)));
  const pdfBytes = await buildPdf(blobs, spec);
  return {
    dataUrl: await bytesToDataUrl(pdfBytes, 'application/pdf'),
    mimeType: 'application/pdf',
  };
...
export async function processHeavyWorkerMessage(
  ...
  try {
    ...
    if (message.type === 'OFFSCREEN_RUN_ML_REDACTION') {
      const result = await runMlRedaction(String(message.dataUrl));
      refs.clear();
      return {
        type: 'OFFSCREEN_RESULT',
        id: message.id,
