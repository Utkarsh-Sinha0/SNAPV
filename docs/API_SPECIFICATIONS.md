...
| `CAPTURE_REGION` | `{ tabId: number, spec: ExportSpec }` or `{ tabId: number, rect: DOMRect, spec: ExportSpec }` | Without `rect`, starts the interactive overlay. With `rect`, completes the crop path. |
...
| `RUN_ML_REDACTION` | `{ captureId: string }` | Routes to the local ML path via the heavy-worker core |
